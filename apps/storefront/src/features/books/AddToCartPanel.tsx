...
export default function AddToCartPanel({ book }: { book: Book }) {
  ...
  const maxQty =
    book.format === "digital"
      ? 10
      : Math.min(book.inventoryCount ?? 0, 10);
  ...
  return (
    <div className="space-y-4">
        ...
        <p className="font-sans text-sm text-ink-muted">
          {inStock ? (
            book.inventoryCount <= 3 ? (
              <span className="text-gold">Only {book.inventoryCount} left</span>
            ) : (
      ...
      {book.format === "digital" ? (
        ...
        <p className="font-sans text-sm text-ink-muted">
          Ships in 3–5 business days · Shipping collected on secure payment page
        </p>
      )}
